...
__purpose__ = (
    "Submit a folder to a specific entity for evaluation for the "
...
def parse_args():
    ...
    parser.add_argument(
        "-p", "--password", dest="password", required=True, help="Your Synapse password"
    )
    parser.add_argument(
        "-t",
        "--team",
        dest="teamname",
        required=True,
        help="Your team" "s name as you would like it to appear" " on the leaderboard",
    )
    parser.add_argument(
        "-n",
        "--name",
    ...
    parser.add_argument(
        ...
        required=True,
        type=int,
        help="The challenge that you want to submit"
...
if __name__ == "__main__":
    ...
    ID_DICT = {
        3257313: "syn3249110",  # 2015 Abdomen Challenge STD
        ...
        3567571: "syn3270357",
    }  # 2013 CAP Free Challenge

    if ARGS.challenge not in ID_DICT.keys():
        sys.exit(
            "ERROR: The challenge type %i is not one of the following %s"
    ...
    try:
        ...
        sys.exit(
            "ERROR: Could not log in with credentials. "
            "Please check username and password"
        )

    PARTICIPANT_FILE = SYN.store(File(ARGS.directory, parent=ID_DICT[ARGS.challenge]))
