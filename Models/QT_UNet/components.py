...
class CAWindowAttention3D(nn.Module):
    """ Window based multi-head self attention (W-MSA) module with relative position bias for cross-attention.
    It supports both of shifted and non-shifted window.
    Args:
        dim (int): Number of input channels.
        window_size (tuple[int]): The temporal length, height and width of the window.
    ...
    def forward(self, x, mask, cross_x):
        B_, N, C = x.shape
        q = (
            self.q(x)
        ...
        x2 = (attn @ v).transpose(1, 2).reshape(B_, N, C)
        x2 = self.proj(x2)
...
class SwinTransformerBlock3D(nn.Module):
    ...
    def __init__(
        ...
        if is_decoder:
            self.cross_attn = CAWindowAttention3D(
                dim,
                window_size=self.window_size,
                num_heads=num_heads,
                qkv_bias=qkv_bias,
    ...
    def caMHSA(self, x, mask_matrix, cross_x):
        B, D, H, W, C = x.shape
        window_size, shift_size = get_window_size(
            (D, H, W), self.window_size, self.shift_size
        )
        ...
        pad_r = (window_size[2] - W % window_size[2]) % window_size[2]
        x = F.pad(x, (0, 0, pad_l, pad_r, pad_t, pad_b, pad_d0, pad_d1))
        cross_x = F.pad(cross_x, (0, 0, pad_l, pad_r, pad_t, pad_b, pad_d0, pad_d1))
        _, Dp, Hp, Wp, _ = x.shape
        # cyclic shift
        if any(i > 0 for i in shift_size):
            shifted_x = torch.roll(
        ...
        # partition windows
        x_windows = window_partition(shifted_x, window_size)  # B*nW, Wd*Wh*Ww, C
        cross_x_windows = window_partition(
            shifted_cross_x, window_size
        )  # B*nW, Wd*Wh*Ww, C

        cross_attn_windows = self.cross_attn(
            x_windows, mask=attn_mask, cross_x=cross_x_windows
        )  # B*nW, Wd*Wh*Ww, C
        # merge windows
    ...
    def forward(self, x, mask_matrix, cross_x=None):
        """ Forward function.
        ...
        if self.is_decoder:
            if self.use_checkpoint:
                x2 = checkpoint.checkpoint(self.caMHSA, x, mask_matrix, cross_x)
            else:
                x2 = self.caMHSA(x, mask_matrix, cross_x)
...
class PatchMerging(nn.Module):
    ...
    def forward(self, x):
        ...
        x_back = torch.cat([x0, x1, x2, x3], -1)  # B D/2 H/2 W/2 4*C

        x0 = x[:, 1::2, 0::2, 0::2, :]  # B D/2 H/2 W/2 C
        x1 = x[:, 1::2, 1::2, 0::2, :]  # B D/2 H/2 W/2 C
        x2 = x[:, 1::2, 0::2, 1::2, :]  # B D/2 H/2 W/2 C
        x3 = x[:, 1::2, 1::2, 1::2, :]  # B D/2 H/2 W/2 C
        x_front = torch.cat([x0, x1, x2, x3], -1)  # B D/2 H/2 W/2 4*C

        x = torch.cat([x_back, x_front], -1)  # B D/2 H/2 W/2 8*C
...
class PatchExpand(nn.Module):
    def __init__(self, input_resolution, dim, dim_scale=2, norm_layer=nn.LayerNorm):
        ...
        self.expand = (
            nn.Linear(dim, 4 * dim, bias=False) if dim_scale == 2 else nn.Identity()
        )
...
class BasicLayer_up(nn.Module):
    ...
    def forward(self, x, cross_x):
        ...
        cross_x = rearrange(cross_x, "b c d h w -> b d h w c")
...
class SwinTransformerSys3D(nn.Module):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.encoder_output_dim = [
            self.get_input_dim(embed_dim, 0),
            *self.get_input_resolution(0),
        ]
        ...
        for i_layer in range(self.num_layers):
            concat_linear = (
                nn.Linear(
                    2 * self.get_input_dim(embed_dim, i_layer),
                    self.get_input_dim(embed_dim, i_layer),
                    bias=False,
                )
                if i_layer > 0
                else nn.Identity()
            ...
            if i_layer == 0:
                layer_up = PatchExpand(
                    input_resolution=self.get_input_resolution(i_layer),
                    dim=self.get_input_dim(embed_dim, i_layer),
                    dim_scale=2,
                    norm_layer=norm_layer,
                )
            else:
    ...
    def get_input_dim(self, embed_dim, i_layer):
        return int(embed_dim * 2 ** (self.num_layers - 1 - i_layer))

    def get_input_resolution(self, i_layer):
        return (
            self.patches_resolution[0] // (2 ** (self.num_layers - 1 - i_layer)),
            self.patches_resolution[1] // (2 ** (self.num_layers - 1 - i_layer)),
            self.patches_resolution[2] // (2 ** (self.num_layers - 1 - i_layer)),
        )
    ...
    def forward_up_features(self, x, x_downsample):
        for inx, layer_up in enumerate(self.layers_up):
            ...
            if inx == 0:
                ...
                x = rearrange(x, "n d h w c -> n c d h w")
                x = layer_up(x, x_downsample[-(inx + 1)])
    ...
    def _freeze_stages(self):
        if self.frozen_stages >= 0:
            ...
            for param in self.layer.patch_embed.parameters():
